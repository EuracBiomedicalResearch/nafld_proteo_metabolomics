...
- *Pool*: pool of serum samples from about 5,000 participants of the CHRIS
  study.
- *Pool_Dil*: same as *Pool* samples, but the sample was added/measured in
  different concentrations.
- *Pool_IS_Dil*: *Pool* samples with the internal standards (IS) spiked at
  different concentrations. TODO: need to figure out if only the IS where spiked
  at different concentrations, or also the whole sample was diluted.
...
#' Fit for each standards
standard$mzmin <- standard$mz - 0.02
standard$mzmax <- standard$mz + 0.02
...
```{r}
save(nafld, file = "after_visu.RData")
```
...
```{r findchrompeaks_all_data, echo=TRUE, eval = !file.exists("after_peakdetect.RData")}
nafld <- findChromPeaks(nafld, param = param, chunkSize = 10L)
save(nafld, file = "after_peakdetect.RData")
```

```{r, echo = FALSE, eval = file.exists("after_peakdetect.RData")}
#' Loading the already peak-detected data
load("after_peakdetect.RData")
```
...
```{r remove_me_after_evaluation}
#' DELETEEEE
#' creating matrix for rt alignment
standard <- read.delim("Mix_alignment2.txt")
...
for (i in which(sampleData(nafld)$sample_type != "Blank")) {
    ...
    match_intern_standard <- MetaboAnnotation::endoapply(match_intern_standard,
                                                         FUN = rem_dups)
    ID_table2[, i] <- match_intern_standard$target_peak_id
}

#' Function to create rt dataframe;
#' avoiding subset with NA turns out to be much more efficient
rtdf <- function(nafld, ID_table2) {
    index <- as.vector(ID_table2)
    nna <- !is.na(index)
    x <- rep(NA, length(index))
    x[nna] <- chromPeaks(nafld)[index[nna], "rt"]
    dim(x) <- dim(ID_table2)
    rownames(x) <- rownames(ID_table2)
    colnames(x) <- colnames(ID_table2)
    x
}

#' run for nafld
RT_raw2 <- rtdf(nafld, ID_table2)
```
...
param <- PeakGroupsParam(span = 0.5, # was 0.6
...
```{r second_adjustRtime, message=FALSE, warning=FALSE, eval = FALSE}
#' second run
#' correspondence
...
#' See result
plotAdjustedRtime(nafld, col = paste0(col_sample, alpha))
grid()
legend("topright", col = col_phenotype,
       legend = names(col_phenotype), lty = 1)

plotAdjustedRtime(nafld, col = paste0(col_batches, alpha))
grid()
legend("topright", col = col_batch_id,
       legend = names(col_batch_id), lty = 1)
```

```{r eics_after_alignment, include=FALSE}
#' Plot EICs for the mix standards.
eics_s <- chromatogram(nafld[pool_index],
                       mz = as.matrix(standard[, c("mzmin", "mzmax")]),
                       rt = as.matrix(standard[, c("rtmin", "rtmax")]),
...
#' DELETEEEE
#' get RT table after alignment
RT_aligned2 <- rtdf(nafld, ID_table2)

#' Get RT table only for study samples
index_s <- sampleData(nafld)$sample_type == "Study"

Sdsdf <- data.frame(
    Raw_pool = rowSds(RT_raw2[, pool_index], na.rm = TRUE),
    Aligned_pool = rowSds(RT_aligned2[, pool_index], na.rm = TRUE),
    Raw_study = rowSds(RT_raw2[, index_s], na.rm = TRUE),
    Aligned_study = rowSds(RT_aligned2[, index_s], na.rm = TRUE)
)
...
#' Fit for each standards
standards_all$mzmin <- standards_all$mz - 0.02
standards_all$mzmax <- standards_all$mz + 0.02
standards_all$rtmin <- standards_all$RT - 12
standards_all$rtmax <- standards_all$RT + 12


#' Correspondence results for all standards. This call takes way to long,
#' can only be fixed by a new result object, i.e. Chromatograms.
eics_s <- chromatogram(nafld,
...
for (i in seq_len(nrow(intern_standard))) {
    ...
    if (hasFeatures(eic)) {
        plotChromPeakDensity(
            eic, main = intern_standard$name[i],
            col = paste0(col_batches, 80),
            peakCol = paste0(col_batches[chromPeaks(eic)[, "sample"]], 80),
            peakBg = paste0(col_batches[chromPeaks(eic)[, "sample"]], 20),
            peakPch = 21)
