...
#' Plot again
chromatogram(nafld, aggregationFun = "max", chunkSize = 6) |>
...
pool_idx <- which(sampleData(nafld)$sample_type == "Pool")
batches_pool <- col_batch_id[sampleData(nafld)$batch_id][pool_idx]
...
for (i in seq_len(nrow(intern_standard))) {
    png(paste0(dr, "EIC_", intern_standard$abbreviation[i], ".png"),
        width = 12, height = 8, units = "cm", res = 600, pointsize = 4)
    plot(eics[i, pool_idx], main = intern_standard$name[i],
...
eics_s <- chromatogram(nafld[pool_idx],
...
```{r echo=TRUE}
#' All EICs in pool
eics <- chromatogram(nafld[pool_idx],
                     mz = as.matrix(intern_standard[, c("mzmin", "mzmax")]),
                     rt = as.matrix(intern_standard[, c("rtmin", "rtmax")]))
...
```{r echo=TRUE, fig.cap = "Numbers of detected peaks per sample."}
...
- considerably higher number of detected peaks in the first batches.
- About the same number of peaks detected in QC and study samples of the same
  batch.
- 3 study samples with very low numbers of detected peaks (even less than in
  blanks) -> failed injection or no sample available.
...
bpc_raw <- chromatogram(nafld[pool_index], aggregationFun = "max",
                        chromPeaks = "none")

plot(bpc_raw, main = "BPC of Pool sample",
     col = paste0(batches_pool, 80),
     lwd = 0.8)
grid()
...
for (i in unique(fdef$sample)){
    ...
    match_intern_standard <- MetaboAnnotation::endoapply(match_intern_standard,
                                                     FUN = rem_dups)
    ID_table[,i] <- match_intern_standard$target_feature_id
    }
}
