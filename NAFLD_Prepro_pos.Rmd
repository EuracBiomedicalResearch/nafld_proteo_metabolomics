...
#' redefine the index of the pool
pool_index <- which(sampleData(nafld)$sample_type == "Pool")
...
Second run this time based on correspondence step on Pool samples.
...
dr <- "EIC_standard/Pool_batch/after_second_alignment/"
dir.create(dr, recursive = TRUE, showWarnings = FALSE)
...
pandoc.table(Sdsdf, style = "rmarkdown", split.table = Inf)
```

```{r}
par(mfrow = c(13, 4.5, 1, 0.5))
library(vioplot)
...
pool_sample_raw <- nafld_raw[pool_index]
