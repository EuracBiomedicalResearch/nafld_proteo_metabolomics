...
```{r load-chromPeaks, include=FALSE, eval= file.exists("after_peakdetect")}
load("after_peakdetect")
```

```{r findchrompkeaks all data, echo=TRUE, eval = !file.exists("after_peakdetect")}
...
```{r load-refine, include=FALSE, eval= file.exists("nafld_after_refine")}
load("nafld_after_refine")
```

```{r refine dataset,  message=TRUE, eval = !file.exists("nafld_after_refine")}
...
One run based on a pre-defined peak matrix of IS standard with rt for each
Pool samples
...
With `fillChromPeaks` we could thus rescue signal for all but `r sum(is.na(featureValues(nafld)))`
features.
